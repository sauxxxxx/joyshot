...
export function CameraPreview({ flash, status, stream, videoRef, mirrored = true }: CameraPreviewProps) {
  ...
  return <div className={styles.shell}>
    ...
    <div className={styles.view}>
      ...
      {!stream && <div className={styles.placeholder} role="status"><span className={styles.placeholderIcon} aria-hidden="true">{status === "error" ? <CameraOff size={29} /> : <Camera size={29} />}</span><strong>{status === "requesting" ? "Let us see your smile." : status === "error" ? "The camera needs a hand." : "Opening the booth…"}</strong><span>{status === "error" ? "Check the message below to reconnect." : "Allow Camera when your browser asks. No microphone needed."}</span></div>}
    ...
    <footer><span>FOUR PHOTOS</span><span>ON THIS DEVICE</span><span>{mirrored ? "MIRRORED" : "NORMAL"}</span></footer>
  </div>;
}
