...
import { calculatePhotoLayout } from "@/features/strip/photoLayouts";
...
interface ResultStudioProps {
  ...
  toolsOnly?: "photos" | "formats";
  layout?: PhotoLayoutId;
}

export function ResultStudio({ photos, strip, title, mode, onEditsChange, onRetake, retentionHours = 0, initialEdits, toolsOnly, layout = "strip" }: ResultStudioProps) {
  const [localEdits, setEdits] = useState(() => initialEdits?.length === photos.length ? initialEdits : photos.map(createPhotoEdit));
  const edits = initialEdits?.length === photos.length ? initialEdits : localEdits;
  ...
  const frame = calculatePhotoLayout(photos.length === 8 ? 8 : 4, layout).frames[selected];
  ...
  return (
    <section className={`${styles.studio} ${toolsOnly ? styles.compact : ""}`} aria-label={toolsOnly === "formats" ? "Export formats" : "Photo editor"}>
      {!toolsOnly && <div className={styles.heading}><h2 id="editor-title">Finish it your way.</h2><p>{socialHint}</p></div>}
      {toolsOnly !== "formats" && <>
      ...
      {active && <div className={styles.editorGrid}>
        ...
        }} onPointerUp={() => { drag.current = null; }} onPointerCancel={() => { drag.current = null; }}><CropPreview edit={active} width={frame.width} height={frame.height} label={`Editing photo ${selected + 1}`} /><span>Drag to reposition</span></div>
        <div className={styles.editControls}>
          ...
          <label>Brightness <output>{active.brightness}%</output><input type="range" aria-label="Brightness" min="60" max="140" value={active.brightness} onChange={(event) => update({ brightness: Number(event.target.value) })} /></label>
          <label>Contrast <output>{active.contrast}%</output><input type="range" aria-label="Contrast" min="70" max="150" value={active.contrast} onChange={(event) => update({ contrast: Number(event.target.value) })} /></label>
          <label>Zoom <output>{active.zoom.toFixed(1)}×</output><input type="range" aria-label="Zoom" min="1" max="2" step="0.1" value={active.zoom} onChange={(event) => update({ zoom: Number(event.target.value) })} /></label>
          <label>Horizontal position <output>{active.offsetX}</output><input type="range" aria-label="Horizontal position" min="-50" max="50" value={active.offsetX} onChange={(event) => update({ offsetX: Number(event.target.value) })} /></label>
          <label>Vertical position <output>{active.offsetY}</output><input type="range" aria-label="Vertical position" min="-50" max="50" value={active.offsetY} onChange={(event) => update({ offsetY: Number(event.target.value) })} /></label>
      ...
      {toolsOnly !== "photos" && <div className={styles.exportActions}>
        {!toolsOnly && <button className="button buttonStamp" type="button" disabled={!strip || busy !== null} onClick={() => strip && void run("share", async () => { const shared = await shareImage(strip, "joyshot.png", title); if (!shared) { downloadBlob(await fetch(strip).then((response) => response.blob()), "joyshot.png"); setMessage("Sharing is unavailable here, so the PNG was downloaded."); } })}><Share2 size={18} /> Share</button>}
        <button className="button buttonStamp" type="button" disabled={busy !== null} onClick={() => void run("gif", async () => downloadBlob(await createGif(edits), "joyshot.gif"))}><Film size={18} /> {busy === "gif" ? "Making GIF..." : "GIF"}</button>
        <button className="button buttonStamp" type="button" disabled={busy !== null} onClick={() => void run("video", async () => downloadBlob(await createWebm(edits), "joyshot.webm"))}><Video size={18} /> {busy === "video" ? "Making video..." : "Video"}</button>
        <button className="button buttonStamp" type="button" disabled={!strip || busy !== null} onClick={() => strip && void run("story", async () => downloadBlob(await fetch(await createSocialImage(strip, "story")).then((response) => response.blob()), "joyshot-story.png"))}><RectangleVertical size={18} /> Story</button>
        <button className="button buttonStamp" type="button" disabled={!strip || busy !== null} onClick={() => strip && void run("square", async () => downloadBlob(await fetch(await createSocialImage(strip, "square")).then((response) => response.blob()), "joyshot-square.png"))}><Square size={18} /> Square</button>
        {!toolsOnly && <button className="button buttonStamp" type="button" disabled={!strip || busy !== null} onClick={() => strip && void run("save", async () => { await saveGalleryItem(strip, title, mode, retentionHours); setMessage("Saved privately on this device."); })}><Save size={18} /> Save to gallery</button>}
      </div>}
