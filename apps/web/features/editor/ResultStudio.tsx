  ...
  return (
    <section className={styles.studio} aria-labelledby="editor-title">
      ...
      <div className={styles.exportActions}>
        <button className="button buttonStamp" type="button" disabled={!strip || busy !== null} onClick={() => strip && void run("share", async () => { const shared = await shareImage(strip, "joyshot.png", title); if (!shared) { downloadBlob(await fetch(strip).then((response) => response.blob()), "joyshot.png"); setMessage("Sharing is unavailable here, so the PNG was downloaded."); } })}><Share2 size={18} /> Share</button>
        ...
        <button className="button buttonStamp" type="button" disabled={!strip || busy !== null} onClick={() => strip && void run("save", async () => { await saveGalleryItem(strip, title, mode, retentionHours); setMessage("Saved privately on this device."); })}><Save size={18} /> Save to gallery</button>
      </div>
