...
import { Download, Images, LoaderCircle, Trash2 } from "lucide-react";
...
export function GalleryView() {
  ...
  return <section className={styles.gallery} aria-labelledby="gallery-title">
    ...
    {items.length === 0 ? <div className={styles.empty}><Images size={40} /><h2>No saved strips yet</h2><p>Save a finished booth result and it will appear here.</p></div> : <div className={styles.grid}>{items.map((item) => <article key={item.id}><img src={item.url} alt={item.title} /><div><span>{item.mode} · {new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(item.createdAt)}</span><h2>{item.title}</h2><div className={styles.actions}><a href={item.url} download={`joyshot-${item.id}.png`}><Download size={17} /> Download</a><button type="button" onClick={() => void deleteGalleryItem(item.id).then(load)}><Trash2 size={17} /> Delete</button></div></div></article>)}</div>}
  </section>;
}
